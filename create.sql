...
create table tb_order (number_recurrence integer not null, created_date timestamp(6), id bigint generated by default as identity, last_modified_date timestamp(6), order_rec varchar(255), payment_date varchar(255), subscription_id varchar(255), primary key (id));
create table tb_product (quantity integer not null, total_price numeric(38,2), unit_price numeric(38,2), id bigint generated by default as identity, order_id bigint, sku varchar(255), primary key (id));
...
create table tb_subscription (cancellation_date date, next_billing_date date, next_recurrence integer not null, number_recurrences integer not null, created_at timestamp(6) with time zone not null, updated_at timestamp(6) with time zone not null, customer_id varchar(255), id varchar(255) not null, status varchar(255) check (status in ('AGUARDANDO_PRIMEIRO_PAGAMENTO','ATIVO','PAGAMENTO_PENDENTE','INATIVO_CANCELADO','EXPIRADO','PAUSADO','PAGAMENTO_ATRASADO')), primary key (id));
...
create table tb_webhook_idempotency (created_at timestamp(6) with time zone not null, id bigint generated by default as identity, last_processed_at timestamp(6) with time zone not null, webhook_job_id bigint not null, event_type varchar(255) not null, signature varchar(255) not null unique, source varchar(255) not null, subscription_id varchar(255) not null, primary key (id));
create table tb_webhook_job (attempts integer not null, status smallint not null check (status between 0 and 4), created_at timestamp(6) with time zone not null, id bigint generated by default as identity, next_run_at timestamp(6) with time zone not null, updated_at timestamp(6) with time zone not null, event_type varchar(255) not null, payload TEXT not null, source varchar(255) not null, subscription_id varchar(255) not null, primary key (id));create index idx_year_month on tb_revenue_cache (year_month);
create index idx_webhook_idempotency_created on tb_webhook_idempotency (created_at);
create index idx_webhook_job_next on tb_webhook_job (next_run_at);
